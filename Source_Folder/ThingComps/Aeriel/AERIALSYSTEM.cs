...
namespace AvaliMod
{
	...
	public class AERIALSYSTEM : Building_TurretGun
	{
		...
		public bool Powered
        {
            get
            {
				bool hasNexus = power == null || (power.PowerOn && power.PowerNet.connectors.Any(x => x.parent.def == AvaliDefs.AvaliNexus));
				return hasNexus && power.PowerNet.connectors.Where(x => x.parent.def == AvaliDefs.AvaliNexus).Any(x=>x.parent.TryGetComp<NexusComp>()!=null&&x.parent.TryGetComp<NexusComp>().AERIAlIsEnabled);
			}
        }
        ...
        {
			this.power = this.TryGetComp<CompPowerTrader>();
		...
		public override IEnumerable<Gizmo> GetGizmos()
		{
			...
			if (this.CanSetForcedTarget)
			{
				...
				command_VerbTarget.disabled = !Powered;
				if (base.Spawned && this.IsMortarOrProjectileFliesOverhead && base.Position.Roofed(base.Map))
				{
			...
			if (this.forcedTarget.IsValid)
			{
				...
				command_Action.disabled = !Powered;
