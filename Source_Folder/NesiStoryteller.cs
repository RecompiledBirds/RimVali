...
namespace AvaliMod
{
    ...
    public class NesiStoryTeller : StorytellerComp
    {
        ...
        bool HasPawnsNotAvali{ get { return RimValiCore.RimValiUtility.AllPawnsOfFactionSpawned(Faction.OfPlayer).Any(pawn =>pawn.RaceProps.Humanlike &&  pawn.def != AvaliDefs.RimVali || pawn.def != AvaliDefs.IWAvaliRace); } }
        ...
        int countPawnsavali { get { return RimValiCore.RimValiUtility.AllPawnsOfFactionSpawned(Faction.OfPlayer).Where(pawn => pawn.def == AvaliDefs.RimVali || pawn.def == AvaliDefs.IWAvaliRace).Count(); } }
        int countPawnsNotavali { get { return RimValiCore.RimValiUtility.AllPawnsOfFactionSpawned(Faction.OfPlayer).Where(pawn => pawn.def != AvaliDefs.RimVali || pawn.def != AvaliDefs.IWAvaliRace).Count(); } }
        void UpdateState()
        {
