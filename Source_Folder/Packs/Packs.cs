...
#pragma warning disable CS0114 // Member hides inherited member; missing override keyword
    public class AvaliPack : Thing, ILoadReferenceable, IExposable
    {
        ...
        public void CleanupPack()
        {
            ...
            {
                for(int a = 0; a < pawns.Where(x => x == pawn).Count() - 1; a++)
                {
                    pawns.Remove(pawns.Where(x => x == pawn).ToList()[a]);
                }
            }
        }
        ...
        public void UpdateHediffForAllMembers()
        {
            ...
            {
                foreach (Pawn pawn in pawns.Where(p => !p.Dead&&p.Spawned))
                {
        ...
        {
            if (DefDatabase<AvaliPackSkillDef>.AllDefs.Count() > 0 && this != null && pawns.Count>1)
            {
